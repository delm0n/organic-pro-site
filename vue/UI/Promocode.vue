<template>
  <div class="coupon-container" v-if="validateSum && ordered">
    <input
      ...
      v-bind:class="[
        {
          'input-error': validateError || serverError,
          'input-success': getPromocode.id,
        },
        fashion,
      ]"
    ...
    <transition name="hide">
      <span
        ...
        v-show="getPromocode.id"
        v-text="'Промокод на ' + getPromocode.price + ' применён'"
      ></span>
    </transition>
    ...
    >
      <span
        class="coupon-error"
        v-show="validateError || serverError"
        v-text="message"
      ></span>
    </transition>
    ...
    <template v-if="getPromocode.id">
      ...
      <input
        type="hidden"
        class="costs"
        v-bind:name="'cost[4610]'"
        v-bind:value="'-' + getPromocode.price"
      />
  ...
  methods: {
    ...
    postPromocode: function () {
      ...
      request.addEventListener("load", () => {
        this.setPromocode(request.response);
